...
describe('test/stub.test.js', () => {
  ...
  it('should use stubFn', function* () {
    ...
    assert((yield ctx.service.clz.fn('a', 'b', 'c')) === 'stub clz fn: a,b,c');

    assert((yield ctx.service.clz.real('a', 'b', 'c')) === 'not override: a,b,c');
    assert((yield ctx.service.clz.empty()) === 'origin empty');
    ...
    assert((yield ctx.service.fn.echo()) === 'stub fn: example');
